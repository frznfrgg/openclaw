...
function installRuntime(params?: { storeAllowFrom?: string[] }) {
  setVkRuntime({
    channel: {
      ...
      pairing: {
        readAllowFromStore: mocks.readAllowFromStore.mockImplementation(
          async () => params?.storeAllowFrom ?? [],
        ),
        upsertPairingRequest: mocks.upsertPairingRequest.mockResolvedValue({
          code: "PAIR-1",
          created: true,
        }),
      },
...
describe("routeVkInboundEvent", () => {
  beforeEach(() => {
    ...
    mocks.readAllowFromStore.mockResolvedValue([]);
    mocks.upsertPairingRequest.mockResolvedValue({
      code: "PAIR-1",
      created: true,
    });
  });
