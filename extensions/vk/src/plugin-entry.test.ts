...
describe("VK plugin entrypoints", () => {
  it("publishes the bundled VK channel plugin entry", () => {
    ...
    expect(loadedPlugin.approvalCapability).toBeDefined();
    expect(loadedPlugin.approvalCapability?.authorizeActorAction).toBeTypeOf("function");
  });
  ...
  it("wires native approval capability through the live VK plugin", () => {
    expect(vkPlugin.approvalCapability).toBe(vkApprovalCapability);
  });
